...
def setAssetProperties(assetId, properties):
    ...
    """
    send_('/setproperties',
          {'id': assetId, 'properties': json.dumps(properties)})
...
def send_(path, params, opt_method='POST', opt_raw=False):
    ...
    def send_with_backoff(retries=0):
        ...
        """
        try:
            response, content = http.request(url, method=opt_method,
                                             body=payload,
                                             headers=headers)
            if response.status == 429:
                if retries < MAX_RETRIES:
                    time.sleep(min(2 ** retries * BASE_RETRY_WAIT,
                                   MAX_RETRY_WAIT) / 1000)
    ...
    content_type = (response['content-type'] or 'application/json').split(';')[
        0]
