...
class Serializer(object):
    ...
    def __init__(self, is_compound=True):
        ...
        # A lookup table from object hash to subtree names as stored in
        # self._scope
        self._encoded = {}
    ...
    def _encodeValue(self, obj):
        ...
        If _is_compound is True, this will fill the _scope and _encoded
        properties.
        ...
        if self._is_compound and encoded:
            # Already encoded objects are encoded as ValueRefs and returned
            # directly.
            return {
                'type': 'ValueRef',
                'value': encoded
            ...
            # A raw date slipped through. Wrap it. Calling ee.Date from
            # here would cause a circular dependency, so we encode it manually.
            return {
                'type': 'Invocation',
                'functionName': 'Date',
            ...
            if (not isinstance(result, (list, tuple)) and
                    (not isinstance(result, (dict)) or result[
                        'type'] == 'ArgumentRef')):
                # Optimization: simple enough that adding it to the scope is
            ...
            # Dictionary are encoded recursively and wrapped in a type
            # specifier.
            result = {
                'type': 'Dictionary',
